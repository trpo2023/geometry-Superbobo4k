...
int errorMessage(int position, string message) {
...
int checkNumber(size_t &p, int &j) {

	// If the 1st char is not '+', '-', or digit
	if (line[p] != '+' && line[p] != '-'
		&& !(line[p] >= '0' && line[p] <= '9')) {
		errorMessage(p, "expected Number");
		return 0;
	}
	...
	for (p = p; p < line.length(); p++) {
		...
		if (line[p] == ')' && j == 2) {
			if (line.length() > p + 1) {
				errorMessage(p + 1, "expected end of line");
				return 0;
			}
			int end = p;
			string number = line.substr(start, end - start);
		...
		if (line[p] == '.') {
			...
			if (!(line[p + 1] >= '0' && line[p + 1] <= '9')) {
				errorMessage(p + 1, "expected Number");
				return 0;
			}
	...
	errorMessage(p, "expected input");
	return 0;
}

int parsing(size_t &p) {
	for (int j = 0; j < 3; j++) {
		if (line.length() == p) {
			errorMessage(p, "expected input");
			return 0;
		...
		while (line[p] == ' ') {
			if (line.length() > p + 1) {
				...
				errorMessage(p + 1, "expected input");
				return 0;
			}
		}
		if (j == 2) {
			if (line[p] == ',') {
				...
				cout << p;
				errorMessage(p - 1, "expected ','");
				return 0;
			}

		}
		if (checkNumber(p, j) == 0) {
			return 0;
		}
	}
	while (line[p] == ' ') {
