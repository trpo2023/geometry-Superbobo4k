...
int errorMessage(int position, string message, string line, int circleCounter, double circleArray[][100]) {
...
int print(string line, int circleCounter, double circleArray[][100]) {
...
int circleCalculation(string line, int circleCounter, double circleArray[][100]) {
...
int checkNumber(size_t& p, int& j, string line, int circleCounter, double circleArray[][100]) {
	...
	if (line[p] != '+' && line[p] != '-'
		&& !(line[p] >= '0' && line[p] <= '9')) {
		errorMessage(p, "expected Number", line, circleCounter, circleArray);
		return 0;
	}
	...
	for (p = p; p < line.length(); p++) {
		...
		if (line[p] == ')' && j == 2) {
			if (line.length() > p + 1) {
				errorMessage(p + 1, "expected end of line", line, circleCounter, circleArray);
				return 0;
			}
			int end = p;
			string number = line.substr(start, end - start);
		...
		if (line[p] == '.') {
			...
			if (!(line[p + 1] >= '0' && line[p + 1] <= '9')) {
				errorMessage(p + 1, "expected Number", line, circleCounter, circleArray);
				return 0;
			}
	...
	errorMessage(p, "expected input", line, circleCounter, circleArray);
	return 0;
}
	...
	for (int j = 0; j < 3; j++) {
		...
		while (line[p] == ' ') {
			if (line.length() > p + 1) {
				...
				errorMessage(p + 1, "expected input", line, circleCounter, circleArray);
				return 0;
			}
		}
		if (j == 2) {
			if (line[p] == ',') {
				...
				cout << p;
				errorMessage(p - 1, "expected ','", line, circleCounter, circleArray);
				return 0;
			}
...
int circle(string line, int circleCounter, double circleArray[][100]) {
	if (line[6] != '(') {
		errorMessage(6, "expected '('", line, circleCounter, circleArray);
		return 0;
	}
	if (line.length() == 7) {
		errorMessage(7, "expected input", line, circleCounter, circleArray);
		return 0;
	}
	size_t p = 7;
	...
	if (inFile.is_open()) {
		...
		while (getline(inFile, line)) {
			...
			if (line.rfind("circle", 0) == 0) {
				...
				cout << line << endl << "^" << endl << "expected circle, triangle or polygon at position 0";
				return 0;
			}
		}
		circleCalculation(line, circleCounter, circleArray);
		print(line, circleCounter, circleArray);
	}
	else {
