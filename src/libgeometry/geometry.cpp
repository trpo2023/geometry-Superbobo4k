...
int circleCalculation(int circleCounter, double circleArray[][100])
{
    ...
    for (int g = 0; g <= circleCounter; g++) {
        for (int h = g + 1; h <= circleCounter; h++) {
            if (pow((circleArray[g][2] - circleArray[h][2]), 2)
                        <= pow((circleArray[g][0] - circleArray[h][0]), 2)
                                + pow((circleArray[g][1] - circleArray[h][1]),
                                      2)
                && pow((circleArray[g][0] - circleArray[h][0]),
                       2) + pow((circleArray[g][1] - circleArray[h][1]), 2)
                        <= pow((circleArray[g][2] + circleArray[h][2]), 2)) {
        ...
        double circleArray[][100])
{
    ...
    for (p = p; p < line.length(); p++) {
        ...
        if (line[p] == ')' && j == 2) {
            if (line.length() > p + 1) {
                errorMessage(
                        p + 1, "expected end of line", line, circleCounter);
                return 0;
            }
            int end = p;
            string number = line.substr(start, end - start);
            ...
            if (line[p] == ',') {
                ...
                while (line[p] == ' ') {
                    if (line.length() > p + 1) {
                        ...
                        errorMessage(
                                p + 1, "expected input", line, circleCounter);
                        return 0;
                    }
                }
            } else {
